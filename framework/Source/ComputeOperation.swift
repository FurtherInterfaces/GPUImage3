...
open class ComputeOperation: ImageProcessingOperation {
    ...
    public init(computeKernelName: String, numberOfInputs: UInt = 1, operationName: String = #file) {
        ...
        let (pipelineState, lookupTable) = generateComputePipelineState(device: sharedMetalRenderingDevice,
                                                                        kernelName: computeKernelName,
    ...
    func internalRenderFunction(commandBuffer: MTLCommandBuffer, outputTexture: Texture) {
        commandBuffer.renderQuadCompute(pipelineState: computePipelineState, uniformSettings: uniformSettings, inputTextures: inputTextures, outputTexture: outputTexture)
    }
}
